...
if (supportForm && supportMessage && supportButton) {
  supportForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        supportMessage.textContent = "Message sent successfully.";
        supportForm.reset();
      } else {
        supportMessage.textContent = "Something went wrong. Please try again.";
      }
    } catch (error) {
      console.error("WebShelf Support Error:", error);
      supportMessage.textContent = "Unable to send your message right now.";
    } finally {
      supportButton.disabled = false;
      supportButton.textContent = "Send message";
    }
  });
}
