...
(() => {
  ...
  function mirrorLinksHTML(site) {
    const mirrors = safeArray(site.links).filter((link) => link?.url && normalizeUrl(link.url) !== normalizeUrl(site.url));
  ...
  function screenshotsHTML(site) {
    const screenshots = safeArray(site.screenshots).filter(Boolean);
  ...
  function prepareRow(row) {
    ...
    row.insertAdjacentElement("afterend", buildPreview(site));
  }
