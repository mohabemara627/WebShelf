...
(() => {
  ...
  overlay.hidden = true;
  overlay.innerHTML = `
    <div class="search-modal" role="dialog" aria-modal="true" aria-labelledby="webshelf-search-title">
  ...
  function siteSearchText(site) {
    return {
      ...
      collections: typeof getSiteCollections === "function" ? getSiteCollections(site).join(" ").toLowerCase() : ""
    };
  }
  ...
  function scoreSite(site, query) {
    ...
    if (text.collections.includes(query)) score += 15;
    return score;
  }
  ...
  function renderResults(matches) {
    ...
    results.innerHTML = visible.map((site, index) => {
      ...
      return `
        <a class="search-result" role="option" aria-selected="false" data-search-index="${index}" href="${safeText(site.url)}" target="_blank" rel="noopener noreferrer" data-site-url="${safeText(site.url)}">
  ...
  results.addEventListener("mousemove", (event) => {
    ...
    if (Number.isFinite(index)) setActiveResult(index);
  });
