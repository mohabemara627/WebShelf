...
`maintenance/src/` contains the individual production JS sources. `data.js`, `common.js`, the six `*.bundle.js` files, and `service-worker.js` are generated. Edit `maintenance/service-worker.source.js` and `core.json` for intentional worker changes. `index.html` and the 12 named category pages such as `anime-streaming.html` are generated too: edit `maintenance/templates/home.html`, `maintenance/templates/category.html` and `maintenance/seo.cjs`, then rebuild. Keep the generic `category.html` shell synchronized with template layout changes for offline/local fallback. CSS and other HTML files remain ordinary production files. The build preserves unrelated Vercel settings while regenerating legacy category redirects. Sitemap category URLs regenerate only when category keys change; external destination sites are never added to the sitemap.
...
This ZIP is the maintainable master. For a static deployment publish the root HTML/CSS/JS, images, manifest, robots and sitemap; keep maintenance local. A Vercel deployment can use **Other**, no install command, build command `node maintenance/export.cjs`, and output directory `public`. This copies only production assets without changing them and excludes maintenance, reports and dependencies. The allowlist in `lib.cjs` permits known production root files and non-hidden image files with approved extensions; unknown HTML/JS, archives, notes and symlinks are excluded. Add intentional new production assets to the allowlist and its tests.

The exporter refuses stale generated assets and existing `public/` directories. It stages the copy before publishing `public/`, so a failed copy does not expose a partial export. Move or remove an old export yourself before repeating it. Run the maintenance build/validator before deployment. Never point a production build at an old Catalog Manager export.

The custom 404 page uses root-relative assets and recovery links for the documented root-domain deployment. Its skip link remains fragment-only. If deploying under a subdirectory, configure the 404 paths for that hosting base. Service-worker registration resolves relative to `common.js`, including on nested missing URLs.

GitHub Actions in `.github/workflows/checks.yml` runs validation, regression tests, a reproducible-build check and export on Windows/Linux with Node.js 22/24. This workflow does not deploy the site or submit real forms.
