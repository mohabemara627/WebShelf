...
test('add/remove preserves catalog data and public bytes; failed transactions restore exact bytes',()=>{
 const root=copy();try{const original=L.readCatalog(root),before=hashes(root),catalog=structuredClone(original);catalog[0].sites.push({name:'Temporary',url:'https://example.com/',icon:''});
  ...
  T.commit(catalog,{root});assert.equal(L.flatten(L.readCatalog(root)).length,L.flatten(original).length+1);assert.deepEqual(V.validate(root,{quiet:true,git:false}).errors,[]);T.commit(original,{root});assert.deepEqual(L.readCatalog(root),original);
  // Successful writes deliberately normalize JSON formatting; rollback checks above remain byte-exact.
  const after=hashes(root);delete after['maintenance/catalog.json'];
  const publicBefore={...before};delete publicBefore['maintenance/catalog.json'];assert.deepEqual(after,publicBefore);
 }finally{clean(root);}
});
