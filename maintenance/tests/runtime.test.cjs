...
test('worker stores category HTML separately by query',async()=>{const w=worker();await w.request('/category.html?type=a');await w.request('/category.html?type=b');assert.deepEqual(w.stored.map(r=>r.url),['https://www.webshelf.link/category.html?type=a','https://www.webshelf.link/category.html?type=b']);});
